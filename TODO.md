...
- [ ] **Refine Pubspec**: Improve package tags and description for optimal `pub.dev` SEO before public release.

## Core Features & Enhancements

- [ ] **`mode: random_n`**: Implement a random permutation selector in `MatrixEngine` so users can evaluate exactly `N` distinct random cases from an enormous hyper-parameter space (from original spec).
- [ ] **Conditional Matrix Parameters**: Add support for dependent arguments (e.g. if `video_codec: h265`, only inject `bitrate: 4k`).
- [ ] **Custom Reporter Templates**: Allow users to inject a custom markdown string builder to override how the Markdown report is visually dumped out.
